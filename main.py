...
# Define the translate model and tokenizer names
...
def get_docs_for_plan(plan_number):
    print(f"Getting docs for plan {plan_number}")
...
if __name__ == '__main__':
    ...
    print("downloaded and converted to text")
    # for each doc, clean and split to chunks
    files_cleand_chunks = [clean_and_split(text) for text in files_text]
    print("cleaned and split to chunks")
    # for each chunk in each doc, translate to english
    translated_chunks = [sentencewise_translate(chunk, he_en_translate_pipe) for cleand_chunks in files_cleand_chunks for chunk in tqdm.tqdm(cleand_chunks)]
